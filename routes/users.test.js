...
const {
  ...
  commonAfterAll,
  u1Token,
  u2Token, // Add a token for an admin user
} = require("./_testCommon");
...
describe("POST /users/:username/jobs/:id", function () {
  test("works for same user", async function () {
    ...
    const res = await db.query(
      "SELECT * FROM applications WHERE username = 'u1' AND job_id = 2"
    );
  ...
  test("unauth for non-admins and not same user", async function () {
    const resp = await request(app)
  ...
  test("works for same user", async function () {
    ...
    expect(resp.body).toEqual({
      user: {
        ...
        email: "u1@email.com",
        isAdmin: false,
        jobs: [1], // Assuming the user applied for job with id 1 in test data
      },
    });
  });
